...
export async function generateAndSaveMilestones(
  projectId: string,
): Promise<void> {
  const supabase = createAuthClient();
  const {
  ...
  const skillLevel =
    user.skill_level ??
    (projectRow.complexity === "challenging"
      ? "advanced"
      : projectRow.complexity === "intermediate"
        ? "intermediate"
        : "beginner");
...
export async function getActiveProjects(): Promise<ActiveProject[]> {
  ...
  if (!session)
    throw new Error("You must be logged in to view active projects.");
...
export async function getMilestoneWithTasks(milestoneId: string) {
  ...
  const [projectResult, tasksResult, allMilestonesResult] = await Promise.all([
    ...
    db
      .from("milestones")
      .select("id, order_index, status, title")
...
export async function generateAndSaveTasks(milestoneId: string): Promise<void> {
  ...
  const completedTitles = (completedMilestones ?? []).map(
...
export async function completeTask(
  ...
  if (
    result.outcome === "next_milestone" ||
    result.outcome === "project_complete"
  ) {
