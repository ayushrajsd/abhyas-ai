    ...
    >
      ...
      {open && (
        <div
          ...
          style={{ backgroundColor: "#faf8f4", borderTop: "1px solid #e8e3da" }}
        >
          {resources.map((r, i) => (
            <a
              key={i}
...
function HintShelf({
  ...
  return (
    <div className="space-y-2">
      {Array.from({ length: revealedLevel }).map((_, i) => (
        ...
        >
          ...
          <p
            ...
            style={{ color: "#3d3020", lineHeight: "1.65" }}
          >
            {HINT_VALUES[i + 1]}
          </p>
        </div>
      ...
      {revealedLevel === 3 && (
        <p>
          Still stuck? You have used all three guided hints. In the next step,
          you will be able to describe where you are blocked and request a
          deeper nudge. [Ask for a deeper nudge]
        </p>
      )}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p
          className="text-sm leading-relaxed"
          style={{ color: "#1c4731", lineHeight: "1.65" }}
        >
          {task.done_when}
        </p>
      </div>
      ...
      {/* Hints */}
      <HintShelf
        hints={task.prewritten_hints as { l1: string; l2: string; l3: string }}
      />
    ...
    >
      ...
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: "#5a8a6b" }}>
          {nextMilestoneId
            ? "You unlocked the next milestone."
  ...
  if (completionResult) {
    return (
      <MilestoneCompleteBanner
        nextMilestoneId={
          completionResult.outcome === "next_milestone"
            ? completionResult.milestoneId
            : undefined
        }
  ...
  return (
    <div className="space-y-3">
      {initialTasks.map((task) => {
        if (task.status === "done")
          return <DoneTaskCard key={task.id} task={task} />;
